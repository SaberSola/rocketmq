...
public class CommitLog {
    ...
    private final FlushCommitLogService flushCommitLogService;//刷盘service

    //If TransientStorePool enabled, we must flush message to FileChannel at fixed periods
    private final FlushCommitLogService commitLogService;//异步提交message到MapplerBuffer里

    private final AppendMessageCallback appendMessageCallback;
    private final ThreadLocal<MessageExtBatchEncoder> batchEncoderThreadLocal;
    private HashMap<String/* topic-queueid */, Long/* offset */> topicQueueTable = new HashMap<String, Long>(1024); //所有队列的偏移量
    ...
    private static int calMsgLength(int bodyLength, int topicLength, int propertiesLength) {
        final int msgLen = 4 //TOTALSIZE 该消息条目的总长丢 4字节
            + 4 //MAGICCODE 魔数 四字节 固定值
            + 4 //BODYCRC  消息体crc校验码
            + 4 //QUEUEID  队列Id
            + 4 //FLAG     消息flage
            + 8 //QUEUEOFFSET 消息在该消息队列的偏移量
            + 8 //PHYSICALOFFSET 消息子CommitLog文件中的偏移量
            + 4 //SYSFLAG  消息系统的flag 是否事务 是否压缩消息等
            + 8 //BORNTIMESTAMP 消息生产者调用消息发送api的时间戳
            + 8 //BORNHOST      消息发送者的 ip 端口号
            + 8 //STORETIMESTAMP  消息存储的时间戳
            + 8 //STOREHOSTADDRESS Broker服务器IP + 端口号
            + 4 //RECONSUMETIMES  消息的重试次数
            + 8 //Prepared Transaction Offset  事务消息的物理偏移量
            + 4 + (bodyLength > 0 ? bodyLength : 0) //BODY 消息体长度
            + 1 + topicLength //TOPIC  主题 长度为topicLength
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) { //开始消息存储
        ...
        // on the client)
        msg.setBodyCRC(UtilAll.crc32(msg.getBody()));//压缩消息相关
        // Back to Results
        AppendMessageResult result = null;//拼接commitlog的结果
        ...
        try {
            ...
            // global
            msg.setStoreTimestamp(beginLockTimestamp);//设置这条消息的存储的时间

            if (null == mappedFile || mappedFile.isFull()) { //如果这个内存映射 mappedFile 为null  或者满了
                mappedFile = this.mappedFileQueue.getLastMappedFile(0); // Mark: NewFile may be cause noise 用偏移量0创建第一次commit文件
            }
            if (null == mappedFile) {
                log.error("create mapped file1 error, topic: " + msg.getTopic() + " clientAddr: " + msg.getBornHostString());//再次创建失败的话 说明磁盘空间不足 或者权限不够
                beginTimeInLock = 0;
                return new PutMessageResult(PutMessageStatus.CREATE_MAPEDFILE_FAILED, null);
            }

            result = mappedFile.appendMessage(msg, this.appendMessageCallback);//讲消息追加到mappedFile 文件中
        ...
        // Statistics 统计topic次数 该topic写入的数量
    ...
    public void handleDiskFlush(AppendMessageResult result, PutMessageResult putMessageResult, MessageExt messageExt) {
        ...
        if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            final GroupCommitService service = (GroupCommitService) this.flushCommitLogService;//刷盘Service
            if (messageExt.isWaitStoreMsgOK()) {
                //构建GroupCommitRequest 并放入GroupCommitService 中
                GroupCommitRequest request = new GroupCommitRequest(result.getWroteOffset() + result.getWroteBytes());//封装requet
                service.putRequest(request);//放入刷盘任务中
    ...
    //异步刷盘 每200ms将ByteBuffer新追加的内容 wrotePosition - commitedPosition 的数据提交到MappedByteBuffer中
    ...
    public static class GroupCommitRequest {
        private final long nextOffset; //下个偏移==
        private final CountDownLatch countDownLatch = new CountDownLatch(1); //闭锁
        ...
        public boolean waitForFlush(long timeout) { //
            try {
                this.countDownLatch.await(timeout, TimeUnit.MILLISECONDS); //wait等待刷盘结果
                return this.flushOK;
    ...
    /**
     * GroupCommit Service  同步刷盘任务
     */
    class GroupCommitService extends FlushCommitLogService {
        private volatile List<GroupCommitRequest> requestsWrite = new ArrayList<GroupCommitRequest>();//同步刷盘任务暂存List
        private volatile List<GroupCommitRequest> requestsRead = new ArrayList<GroupCommitRequest>(); // GroupCommitService 每次处理任务的容器

        public synchronized void putRequest(final GroupCommitRequest request) {
            synchronized (this.requestsWrite) {//说盘任务放入到 writelist中
                this.requestsWrite.add(request);//放入到
            }
        ...
        private void doCommit() {
            synchronized (this.requestsRead) { //加锁
                if (!this.requestsRead.isEmpty()) { //不为null
                    for (GroupCommitRequest req : this.requestsRead) {  //遍历获取任务
                        // There may be a message in the next file, so a maximum of
                        // two times the flush
                        boolean flushOK = false;
                        for (int i = 0; i < 2 && !flushOK; i++) {
                            flushOK = CommitLog.this.mappedFileQueue.getFlushedWhere() >= req.getNextOffset(); //上次已经刷盘的偏移量 > 此次拼接的偏移量
                            if (!flushOK) {
                    ...
                    long storeTimestamp = CommitLog.this.mappedFileQueue.getStoreTimestamp();
                    if (storeTimestamp > 0) {
                        CommitLog.this.defaultMessageStore.getStoreCheckpoint().setPhysicMsgTimestamp(storeTimestamp);//更新监测点刷哦按的时间结果
                    }
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                try {
                    this.waitForRunning(10);//每次执行完一批休眠
                    this.doCommit();//开始提交任务并刷盘
                } catch (Exception e) {
                    CommitLog.log.warn(this.getServiceName() + " service has exception. ", e);
                }
            }
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        private final ByteBuffer msgStoreItemMemory;//存储一条消息内容的byteBuffer
        ...
        /**
         *
         * @param fileFromOffset 文件初始偏移量
         * @param byteBuffer     新建的缓冲区
         * @param maxBlank       最大可以写入的文件偏移量
         * @param msgInner       消息实体
         * @return
         */
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // PHY OFFSET
            long wroteOffset = fileFromOffset + byteBuffer.position(); //获取写入当前文件的写指针fileFromOffset +  byteBuffer.position() 当前可以写入文件指针

            this.resetByteBuffer(hostHolder, 8);//写入address地址
            String msgId = MessageDecoder.createMessageId(this.msgIdMemory, msgInner.getStoreHostBytes(hostHolder), wroteOffset); //生成消息Id
            ...
            Long queueOffset = CommitLog.this.topicQueueTable.get(key); //获取该消息队列中的偏移量 该topic下的具体队列的消息偏移量
            if (null == queueOffset) {
                queueOffset = 0L;
            ...
            final byte[] propertiesData =
                msgInner.getPropertiesString() == null ? null : msgInner.getPropertiesString().getBytes(MessageDecoder.CHARSET_UTF8); //序列化消息属性

            final int propertiesLength = propertiesData == null ? 0 : propertiesData.length;//获取属性的长度

            if (propertiesLength > Short.MAX_VALUE) { //消息属性长度限制Short的最大值
                log.warn("putMessage message properties length too long. length={}", propertiesData.length);
                return new AppendMessageResult(AppendMessageStatus.PROPERTIES_SIZE_EXCEEDED);
            }

            final byte[] topicData = msgInner.getTopic().getBytes(MessageDecoder.CHARSET_UTF8); //topic 序列化
            ...
            if (msgLen > this.maxMessageSize) {  //大于最大的消息长度直接返回失败
                CommitLog.log.warn("message size exceeded, msg total size: " + msgLen + ", msg body size: " + bodyLength
                    + ", maxMessageSize: " + this.maxMessageSize);
                return new AppendMessageResult(AppendMessageStatus.MESSAGE_SIZE_EXCEEDED);
            }

            // Determines whether there is sufficient free space END_FILE_MIN_BLANK_LENGTH 高四节存储文件剩余空间 低四节存储魔数
            if ((msgLen + END_FILE_MIN_BLANK_LENGTH) > maxBlank) { //如果消息长度  +  END_FILE_MIN_BLANK_LENGTH > 空闲空间
                this.resetByteBuffer(this.msgStoreItemMemory, maxBlank); //会重新创建CommitLog 文件
                // 1 TOTALSIZE
            ...
            this.resetByteBuffer(msgStoreItemMemory, msgLen); //初始化消息存储空间
            // 1 TOTALSIZE
            ...
            //上面就是把消息所有的相关信息写入 ByteBuffer
            //记录starttime 开始时间
            final long beginTimeMills = CommitLog.this.defaultMessageStore.now();
            // Write messages to the queue buffer
            //写入共享区的ByteBuffer 此时消息已经写入成功
